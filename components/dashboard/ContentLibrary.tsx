...
export default function ContentLibrary() {
  ...
  const handleGenerateMCQs = async (contentId: number) => {
    ...
    try {
      ...
      if (response.ok) {
        showSuccess(
          "MCQ generation started! Check the MCQ Generator page for status.",
        );

        setContentData((prev) =>
          prev.map((item) =>
            item.id === contentId ? { ...item, status: "generating" } : item,
          ),
        );
      } else {
  ...
  const validateAndSetFile = (file: File) => {
    ...
    if (
      ...
      return showError("Only PDF and PPTX files are supported");
    if (file.size > 10 * 1024 * 1024)
      return showError("File size must be less than 10MB");
  ...
  const handleDrop = (e: React.DragEvent) => {
    e.preventDefault();
    setIsDragOver(false);
    e.dataTransfer.files.length && validateAndSetFile(e.dataTransfer.files[0]);
  };
  ...
  const handleDownload = async (filePath: string, fileName: string) => {
    try {
      const response = await fetch(
        `/api/content/download?path=${encodeURIComponent(filePath)}`,
      );
      if (response.ok) {
  ...
  const filteredContent = contentData.filter(
    (item) =>
      ...
      (selectedFileType === "All" ||
        item.fileType.toUpperCase() === selectedFileType),
  );
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
        <div>
          ...
          <p>Manage your uploaded content and generate MCQs</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          <CardContent className="p-4">
            <div>
              ...
              <p className="text-xl font-bold">{contentData.length}</p>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <div>
              ...
              <p className="text-xl font-bold">
                {
                  contentData.filter(
                    (item) => item.fileType.toUpperCase() === "PDF",
                  ).length
                }
              </p>
            </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div>
              <p className="text-sm">PPTX Files</p>
              <p className="text-xl font-bold">
                {
                  contentData.filter(
                    (item) => item.fileType.toUpperCase() === "PPTX",
                  ).length
                }
              </p>
            </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <Card className="w-full max-w-md mx-4">
            ...
            <CardContent className="space-y-4">
              <div
                className={`border-2 border-dashed rounded-lg p-6 text-center cursor-pointer ${isDragOver ? "border-blue-500 bg-blue-50" : "border-gray-300 hover:border-gray-400"}`}
              ...
              >
                {selectedFile ? (
                  <div className="space-y-2">
                    <div className="w-12 h-12 mx-auto rounded bg-gray-100 flex items-center justify-center">
                      <span className="text-sm font-medium">
                  ...
                  <div className="space-y-2">
                    <div
                      className={`w-12 h-12 mx-auto rounded border-2 border-dashed flex items-center justify-center ${isDragOver ? "border-blue-500" : "border-gray-300"}`}
                    >
                      <span className="text-xl">+</span>
                    </div>
                    <p
                      className={`text-sm ${isDragOver ? "text-blue-600" : "text-gray-600"}`}
                    >
                      {isDragOver
                        ? "Drop your file here"
                    ...
                    <p className="text-xs text-gray-400">
                      PDF and PPTX files only (Max 10MB)
                    </p>
                  </div>
                )}
              </div>
